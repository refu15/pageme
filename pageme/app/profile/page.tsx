...
import { matchAnimal, AnimalResult } from "@/lib/data/animal-matching";
...
import { Loader2, Zap, Info, Share2, Download, Check, RefreshCw } from "lucide-react";
import { toPng, toBlob } from 'html-to-image';
import { jsPDF } from 'jspdf';

// --- Type Definitions (Identical to demo/page.tsx) ---
...
export default function ProfilePage() {
    ...
    // Reuse Export Logic (Same as ResultPage)
    const handleExportPNG = async () => {
        ...
        try {
            ...
            const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = filename; document.body.appendChild(link); link.click(); document.body.removeChild(link); setTimeout(() => URL.revokeObjectURL(url), 100);
        } catch (err) { console.error(err); alert('保存に失敗しました'); } finally { setIsExporting(false); setIsExportPreviewOpen(false); }
    };
    ...
    if (loading) return <div className="min-h-screen flex items-center justify-center bg-[#FAFAFA]"><Loader2 className="w-8 h-8 animate-spin text-[#0A0A0A]" /></div>;
    if (error || !profileData || !matchedResult || !aiInsights) return <div className="min-h-screen flex items-center justify-center bg-[#FAFAFA] text-[#0A0A0A]">{error || 'データ読み込みエラー'}</div>;
    ...
    return (
        <div className="min-h-screen bg-[#FAFAFA] text-[#0A0A0A] relative selection:bg-[#E63946] selection:text-white">
            ...
            <div className="fixed top-4 right-4 z-50 flex gap-2">
                <button onClick={handleShare} className="bg-white border-2 border-[#0A0A0A] p-3 hover:bg-[#F0F0F0] transition-all shadow-[4px_4px_0px_0px_rgba(10,10,10,1)] hover:translate-y-[2px] rounded-full" title="共有URLをコピー">
            ...
            <section className="min-h-screen flex flex-col items-center justify-center px-6 py-20 relative z-10">
                ...
                {/* Profile Page specific action */}
                <div className="flex gap-4 mb-12">
                    <button onClick={() => setIsExportPreviewOpen(true)} className="flex items-center gap-2 px-6 py-3 bg-[#0A0A0A] text-white font-bold border-2 border-[#0A0A0A] shadow-[4px_4px_0px_0px_rgba(200,80,80,1)] hover:translate-y-0.5 transition-all">
                        <Download className="w-4 h-4" /> カード保存
                    </button>
                    <button onClick={() => router.push('/onboarding')} className="flex items-center gap-2 px-6 py-3 border-2 border-[#0A0A0A] text-[#0A0A0A] font-bold hover:bg-[#F0F0F0] transition-all">
                        <RefreshCw className="w-4 h-4" /> 再診断する
                    </button>
                </div>
            ...
            {/* ... (Same sections as ResultPage) -> Optimized for brevity, repeating essential components */}
            {aiInsights.detailed_bio && (<><div className="flex justify-center py-8 relative z-10"><BrushWave className="w-64 text-[#0A0A0A]" /></div><ExpandableBio text={aiInsights.detailed_bio} parameters={aiInsights.parameters || []} /></>)}
            ...
            <section className="px-6 py-20 max-w-3xl mx-auto relative z-10">
                ...
                <div className="space-y-8">{aiInsights.hidden_strengths.map((s, i) => (<div key={i} className="flex gap-4"><div className="w-2 shrink-0 pt-1"><BrushVerticalLine className="h-full w-full text-[#E63946]" /></div><div className="flex-1"><h3 className="text-xl font-bold text-[#0A0A0A] mb-2">{s.title}</h3><p className="text-[#444] leading-relaxed">{s.description}</p></div></div>))}</div>
            </section>
            ...
            <footer className="py-20 text-center relative z-10">
                ...
                <div className="mt-4 flex flex-col gap-2">
                    <a href="/" className="text-[#888] hover:text-[#E63946] transition-colors underline">トップへ戻る</a>
                    <button onClick={async () => { await supabase.auth.signOut(); router.push('/'); }} className="text-[#888] hover:text-[#E63946] transition-colors underline">ログアウト</button>
                </div>
            </footer>
